...
+ `类型检查`
    ...
    > ~~<u>**const常量具有类型，编译器可以进行`安全检查`；#define宏定义没有数据类型，只是简单的字符串替换，不能进行安全检查。**</u>~~感谢两位大佬指出这里问题，见：[issue](https://github.com/Light-City/CPlusPlusThings/issues/5)
...
> 小结：<br>可以发现未被const修饰的变量不需要extern显式声明！而const常量需要显式声明extern，并且需要做初始化！因为`常量在定义后就不能被修改，所以定义时必须初始化`。</font></p>
...
我们不能使用指向const对象的指针修改基础对象，然而如果该指针指向了`非const对象`，可用其他方式修改该非const对象。`可以修改const指针所指向的值的，只是不能通过const对象指针来进行而已`！如下修改：
